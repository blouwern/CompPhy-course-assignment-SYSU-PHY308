...
//   Collective communication times ~ log(P) due to tree-based communication
// algorithm.
//   T_tot = alpha * log(P) + beta * (M*K + K*M) / P
//   alpha is the communication overhead, beta is the overhead of computation &
// data parallelization.

// Main advantage: Predictable performance, easier to debug and maintain Almost
// the most robust, unless the memory is limited or the computation time is
// unpredictable/prolonged.

// Potential bottleneck: have to wait for the lowest process to complete
// computing. May be slowed down when the time cost of each task is
// unpredicatble and much longer than that of MPI index communication.
...
int main(int argc, char *argv[]) {
  ...
  int num_processor;
  int myrank;
  double swtime, ewtime, swtime_seq, ewtime_seq;
  ...
  if (myrank == 0) {
    ...
    MPI_Request *send_requests =
        (MPI_Request *)malloc((num_processor - 1) * sizeof(MPI_Request));
    for (int worker_rank = 1; worker_rank < num_processor; worker_rank++) {
      int n_row_send = worker_rank <= n_remainder_task_row ? n_avg_task_row + 1
                                                           : n_avg_task_row;
    ...
    MPI_Request *recv_requests =
        (MPI_Request *)malloc((num_processor - 1) * sizeof(MPI_Request));
    for (int worker_rank = 1; worker_rank < num_processor; worker_rank++) {
      int n_row_recv = worker_rank <= n_remainder_task_row ? n_avg_task_row + 1
                                                           : n_avg_task_row;
#if NON_BLOCKING_COMMUNICATION
    ...
    printf("MPI Total time taken: %f seconds\n", ewtime - swtime);
    ...
    cblas_dgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, n_matrix_A_row,
                ...
                matrix_result_seq[0], n_matrix_B_col);
    ewtime_seq = MPI_Wtime();
    printf("CBLAS Total time taken: %f seconds\n", ewtime_seq - swtime_seq);
    printf("CBLAS result:");
    print_matrix_less(matrix_result_seq, n_matrix_A_row, n_matrix_B_col, 5, 5);
    printf("Checking result from MPI by CBLAS result:\n");
    check_matrix_equal(matrix_result, matrix_result_seq, n_matrix_A_row,
                       n_matrix_B_col);
  }
  if (myrank != 0) {
    double *L_B =
        (double *)malloc(n_matrix_B_row * n_matrix_B_col * sizeof(double));
    MPI_Bcast(L_B, n_matrix_B_row * n_matrix_B_col, MPI_DOUBLE, 0,
              MPI_COMM_WORLD);

    int n_avg_task_row = n_matrix_A_row / (num_processor - 1);
    int n_remainder_task_row = n_matrix_A_row % (num_processor - 1);
    int n_row_recv =
        myrank <= n_remainder_task_row ? n_avg_task_row + 1 : n_avg_task_row;

    double *L_A_recv =
        (double *)malloc(n_row_recv * n_matrix_A_col * sizeof(double));
    ...
    double *L_result = matrix_multiply_matrix_linear_cblas(
        L_A_recv, L_B, n_row_recv, n_matrix_A_col, n_matrix_B_col);
#else
    double *L_result = matrix_multiply_matrix_linear(
        L_A_recv, L_B, n_row_recv, n_matrix_A_col, n_matrix_B_col);
#endif
    ...
    MPI_Send(L_result, n_row_recv * n_matrix_B_col, MPI_DOUBLE, 0, myrank,
             MPI_COMM_WORLD);
