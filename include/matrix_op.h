...
// **PERFORMANCE-CRITICAL** interface: cblas is way more faster than rough
// simple computing especially for matrix with large magnitude of size.
...
// Matrix is stored sequentially row-by-row and the multiplication is simply
// manual array indexing [i * n_col + j]. Therefore the nested three-loop matrix
// multiplication in C operates at O(N^3) and exhibits poor cache locality for
// large matrices (i.e. data is highly unpredictable for CPU).
